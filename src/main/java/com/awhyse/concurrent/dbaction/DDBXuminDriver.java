...
public class DDBXuminDriver {
	...
	static{
		setDataMap("jdbc:mysql://172.17.5.71:6000/pris_test_new","pris_test_18888","pris_test_18888","test");//172.17.5.71:6000适配层套接字非真实mysql
...
遇到数据库死锁。以前遇到过后端存储过程做了锁表操作，导致前台集群中连接池全都被block住，后续的业务处理因为拿不到链接所有都处理失败了。
		...
		ds.setRemoveAbandonedTimeout(10);//回收废弃时间s120秒
	...
	private static void testBasicDataSourceJDBCTemp(final BasicDataSource ds) {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				System.err.println(temp+" 执行完毕"+map);
			}
		};
		//-----------
