...
public class LengthBaseEncoderNew extends MessageToByteEncoder<String> {
	 ...
	 */
	public  byte[] getMyEncodePack(String msg) {
		byte[] bodyB = msg.getBytes();
		int lengthBody = bodyB.length;
		//-------------------
		byte[] tar = new byte[lengthBody+headSize];
		byte[] headBytes = ByteAndInt.toByteArray(lengthBody, 4);
		System.arraycopy(headBytes, 0, tar, 0, 4);//前面四位写入包长
		tar[4] = 0;//1个字节压缩类型（0为不压缩）
		tar[5] = 0;//1字节加密类型（0为不加密）
		tar[6] = 0;//1个字节序列化方法（0为json）
		tar[7] = 0;//1个字节保留
		System.arraycopy(bodyB, 0, tar,headSize, lengthBody);
		return tar;
	}
