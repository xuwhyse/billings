...
@Service
public class DatumMngImpl implements DatumMng{
	...
	public int getTotalNumber(String searchname) {
		String sql ="SELECT COUNT(*) FROM tb_datum ";
		return jdbcTemplate.queryForInt(sql);
	}

	@Override
	public List<DatumBean> getInfoByPage(int pageIndex,String searchname) {
		DatumBeanCriteria example = new DatumBeanCriteria();
//		if(searchname!=null && !searchname.equals(""))
//			example.createCriteria().andd(searchname);
		example.setOrderByClause(" create_datetime DESC ");
		int pageSize = Constant.PER_PAGE_COUNT;
		RowBounds rowBounds = new RowBounds((pageIndex-1)*pageSize, pageSize);
		return datumBeanMapper.selectByExampleWithRowbounds(example, rowBounds);
	}
