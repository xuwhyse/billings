...
;(function($, undefined) {
	...
	$[pluginName] = function(message, type) {
		...
		if ($[pluginName].options.animate) {
			...
			$('<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>').appendTo($selector)
	...
	$(document).on('click.' + pluginName, '[data-trigger="message"]', function() {
		$(this)[pluginName]();
		if ($(this).is('a')) {
			return false;
		}
	}).on('click.' + pluginName, '[data-dismiss="message"]', function(e) {
		e.preventDefault();
		$(this).closest('.message').trigger('close');
	});
	setTimeout(function(){
		$("."+$[pluginName].options.okClass).slideUp('fast');
